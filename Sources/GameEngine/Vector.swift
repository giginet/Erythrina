...
public struct Vector {
    ...
    /// 複合代入演算子: スカラー倍
    public static func *= (lhs: inout Vector, scalar: Double) {
        lhs.x *= scalar
        lhs.y *= scalar
    }
    ...
    /// 指定された角度（度）で回転
    public func rotated(byDegrees degrees: Double) -> Vector {
        rotated(by: degrees * .pi / 180.0)
    }

    /// 指定された長さにリサイズ
    public func resized(to length: Double) -> Vector {
        normalized * length
    }

    /// 各成分を指定範囲にクランプ
    public func clamped(min: Double, max: Double) -> Vector {
        Vector(
    ...
    /// 長さを指定範囲にクランプ
    public func clampedLength(min: Double, max: Double) -> Vector {
        let len = length
        if len < min {
    ...
    /// 他のベクトルとの距離の二乗
    public func distanceSquared(to other: Vector) -> Double {
        (self - other).lengthSquared
    }

    /// 線形補間
    public func lerp(to other: Vector, t: Double) -> Vector {
        self + (other - self) * t
    }

    /// 2つのベクトル間の角度（ラジアン）
    public func angle(to other: Vector) -> Double {
        let dotProduct = dot(other)
        let lengths = length * other.length
        guard lengths > 0 else { return 0 }
        return acos(Swift.min(Swift.max(dotProduct / lengths, -1), 1))
    }
}
...
extension Vector {
    ...
    /// 角度から単位ベクトルを生成（ラジアン）
    public static func fromAngle(_ radians: Double) -> Vector {
        Vector(x: cos(radians), y: sin(radians))
    }

    /// 角度から単位ベクトルを生成（度）
    public static func fromAngleDegrees(_ degrees: Double) -> Vector {
